...
mongoose.connect(dbURI,{useNewUrlParser:true, useUnifiedTopology: true})
    .then((result)=>{
        ...
        function a1(){
            ...
            lr.eachLine('./IDF_val.txt', (line, last) => {
                Idf[cnt] = Number(line) ;
                cnt += 1 ;
            })
        }
            ...
            Question.find({_id:{$in:ind}})
                .then((result)=>{
                    result.forEach((question)=>{
                        ...
                        TFIDF_value[cnt] = question.vec ;
                        //mag[cnt] = question.magnitude ;
                        mag[cnt] = question.magnitude1 ;
                        //console.log(cnt) ;
                        cnt += 1 
                    })
...
app.post('/search-result',(req,res)=>{
    ...
    for( var l=0 ; l<1791 ; l++ ){
        ...
        for( var m=0 ; m<TFIDF_index[l].length ; m++ ){
            s1 += updated[TFIDF_index[l][m]]*TFIDF_value[l][m] ;
        }
        sorted[l] = s1/Math.sqrt(qmag*mag[l]) ;
